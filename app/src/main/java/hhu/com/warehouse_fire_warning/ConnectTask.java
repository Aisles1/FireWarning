...
import hhu.com.warehouse_fire_warning.util.FROSmoke;
import hhu.com.warehouse_fire_warning.util.FROTemHum;
import hhu.com.warehouse_fire_warning.util.SendMailToSb;
import hhu.com.warehouse_fire_warning.util.StreamUtil;
            ...
            try {
                ...
                if (smokeSocket != null && temHumSocket != null && fanSocket != null) {
                    ...
                    if (Const.linkage && Const.tem > Const.temMaxLim && Const.hum < Const.humMinLim
                        ...
                        hhu.com.warehouse_fire_warning.entity.Log log = new hhu.com.warehouse_fire_warning.entity.Log();
                        log.setTime(time);
                        log.setHum(Const.hum);
                        log.setTem(Const.tem);
                        log.setSmoke(Const.smoke);
                        log.save();

                        try {
                            SendMailToSb.SendMail(simpleDateFormat.format(date), "时间为： " + log.getTime()
                                    + "温度： " + log.getTem() + "湿度： " + log.getHum() + "烟雾值： " + log.getSmoke(), 30);
                        } catch (Exception e) {
                            e.printStackTrace();
                            Log.d("ConnectTask", "联网失败");
                        }
                        // 风扇
                        if (!Const.isFanOn) {
                            Const.isFanOn = true;
